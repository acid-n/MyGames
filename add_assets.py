...
ASSETS = {
    ...
    "cc0_spaceship_sprites": {
        ...
        "destination_path": os.path.join(
            TEMP_DIR, "200-cc0-spaceship-sprites-unzipped"
        ),
        "files_to_copy": [],
    },
}
...
def extract_zip(zip_path, extract_to_path):
    ...
    try:
        ...
        print(
            f"Ошибка: {zip_path} не является действительным ZIP-файлом. Убедитесь, что он не поврежден."
        )
        return False
...
def copy_file(source, destination):
    ...
    try:
        ...
        print(
            f"Ошибка: Исходный файл не найден: {source}. Убедитесь, что он существует."
        )
        return False
    except Exception as e:
...
def get_sprite_coordinates_from_xml(xml_path):
    ...
    try:
        ...
        print(
            f"  Ошибка: XML-файл '{xml_path}' не найден. Невозможно получить координаты спрайтов."
        )
    except Exception as e:
...
def extract_sprite_from_sheet(
    sheet_image_path, sprite_name, sprite_coords, output_path
):
    """Вырезает спрайт из изображения, используя координаты из словаря."""
    if sprite_name not in sprite_coords:
        print(
            f"  Ошибка: Координаты для спрайта '{sprite_name}' не найдены в XML. Пропускаем."
        )
        return False
...
def main():
    ...
    for key, asset_info in ASSETS.items():
        ...
        if not os.path.exists(source_temp_file_path):
            print(
                f"  Внимание: Файл '{source_temp_file_path}' не найден. Пропускаем обработку этого ассета."
            )
            continue
        ...
        if asset_info.get("is_direct_copy", False):
            ...
            if extract_zip(source_temp_file_path, destination_unzip_dir):
                if key == "kenney_planets_pack":  # Планеты
                    planets_source_dir = os.path.join(
                        destination_unzip_dir, asset_info["extract_path"]
                    )
                    if os.path.exists(planets_source_dir):
                        for src_name, dest_name in asset_info["files_to_copy_mapping"]:
                            copy_file(
                                os.path.join(planets_source_dir, src_name),
                                os.path.join(
                                    ASSETS_BASE_DIR, "gfx", "planets", dest_name
                                ),
                            )
                        print("  Планеты успешно скопированы.")
                    else:
                        print(
                            f"  Ошибка: Не найдена папка '{asset_info['extract_path']}' в распакованном архиве {source_temp_file_path}."
                        )

                elif (
                    key == "space_shooter_redux"
                ):  # Space Shooter Redux - основной источник!
                    redux_base_dir = destination_unzip_dir
                    sheet_image_path = os.path.join(
                        redux_base_dir, "Spritesheet", "sheet.png"
                    )
                    sheet_xml_path = os.path.join(
                        redux_base_dir, "Spritesheet", "sheet.xml"
                    )
                    ...
                    if os.path.exists(sheet_xml_path):
                        ...
                        print(
                            f"  Ошибка: XML-файл спрайтшита '{sheet_xml_path}' не найден. Невозможно вырезать спрайты."
                        )
                        continue

                    if not os.path.exists(sheet_image_path):
                        print(
                            f"  Ошибка: Спрайтшит 'sheet.png' не найден в распакованном архиве Space Shooter Redux. Графические ассеты не скопированы."
                        )
                        continue

                    print(
                        f"  Обнаружен спрайтшит: {sheet_image_path} и XML: {sheet_xml_path}. Вырезаем спрайты и копируем звуки..."
                    )
                    ...
                    if os.path.exists(bonus_sounds_dir):
                        copy_file(
                            os.path.join(bonus_sounds_dir, "sfx_laser1.ogg"),
                            os.path.join(
                                ASSETS_BASE_DIR, "audio", "sfx", "laser", "laser01.ogg"
                            ),
                        )
                        copy_file(
                            os.path.join(bonus_sounds_dir, "sfx_twoTone.ogg"),
                            os.path.join(
                                ASSETS_BASE_DIR,
                                "audio",
                                "sfx",
                        ...
                        copy_file(
                            os.path.join(bonus_sounds_dir, "sfx_shieldUp.ogg"),
                            os.path.join(
                                ASSETS_BASE_DIR,
                                "audio",
                                "sfx",
                        ...
                        copy_file(
                            os.path.join(bonus_sounds_dir, "sfx_lose.ogg"),
                            os.path.join(
                                ASSETS_BASE_DIR,
                                "audio",
                                "sfx",
                        ...
                        print(
                            "  Звуковые эффекты (laзер, бонус, щит, один взрыв) из Space Shooter Redux скопированы."
                        )
                    else:
                        print(
                            f"  Внимание: Папка 'Bonus' со звуками не найдена в распакованном архиве Space Shooter Redux. Звуки не скопированы."
                        )

                    # 2. Вырезаем спрайты из sheet.png
                    if sprite_coords:
                        # Корабль игрока
                        extract_sprite_from_sheet(
                            sheet_image_path,
                        ...
                        extract_sprite_from_sheet(
                            ...
                            sprite_coords,
                            os.path.join(
                                ASSETS_BASE_DIR, "gfx", "powerups", "powerup_bolt.png"
                            ),
                        )
                        extract_sprite_from_sheet(
                            sheet_image_path,
                            ...
                            sprite_coords,
                            os.path.join(
                                ASSETS_BASE_DIR, "gfx", "powerups", "powerup_shield.png"
                            ),
                        )
                        extract_sprite_from_sheet(
                            sheet_image_path,
                        ...
                        extract_sprite_from_sheet(
                            ...
                            sprite_coords,
                            os.path.join(
                                ASSETS_BASE_DIR, "gfx", "ui", "frames", "blue_panel.png"
                            ),
                        )
                        ...
                        print(
                            "  Внимание: Используется 'meteorBrown_big1.png' как фон. Возможно, в папке 'Backgrounds' Space Shooter Redux есть более подходящие фоны, которые можно скопировать вручную."
                        )

                    else:
                        print(
                            "  Невозможно вырезать спрайты из sheet.png, так как координаты не загружены."
                        )

                elif (
                    key == "cc0_spaceship_sprites"
                ):  # Для дополнительного разнообразия пришельцев
                    ...
                    if aliens_source_dir:
                        alien_ships = [
                            f
                            for f in os.listdir(aliens_source_dir)
                            if f.lower().endswith(".png")
                        ]

                        if len(alien_ships) >= 12:
                            print(
                                f"  Копируем 12 дополнительных кораблей пришельцев из '{asset_info['source_temp_file']}'."
                            )
                            for i, ship_file in enumerate(alien_ships[:12]):
                                ...
                                dest_path = os.path.join(
                                    ...
                                    "ships",
                                    "aliens",
                                    f"alien_ship_{i+13:02d}.png",
                                )
                                copy_file(src_path, dest_path)
                        else:
                            print(
                                f"  Внимание: В '{asset_info['source_temp_file']}' найдено только {len(alien_ships)} кораблей. Ожидалось 12 дополнительных. Копируем все найденные."
                            )
                        ...
                        print(
                            f"  Ошибка: Не найдена подходящая директория с PNG-кораблями пришельцев в {destination_unzip_dir}. Проверьте структуру архива."
                        )
                else:
                    print(
                        f"  Нет специальной логики копирования для '{key}'. Если ожидались файлы, убедитесь, что они указаны в 'files_to_copy_mapping'."
                    )
...
"""
    if not os.path.exists(gitignore_path):
        ...
        with open(gitignore_path, "r+") as f:
            ...
            if changed:
                print(
                    f"Обновлен файл {gitignore_path} для игнорирования временных файлов и venv."
                )
    ...
    if run_git_command('git commit -m "Добавлены все необходимые игровые ассеты"'):
        ...
        print(
            "Пожалуйста, убедитесь, что у вас настроены учетные данные Git для 'git push'."
        )
