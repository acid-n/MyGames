...
class SpaceObject(pygame.sprite.Sprite):
    ...
    def __init__(self, screen_width, screen_height, image_path, target_size_ratio_range=(0.08, 0.12), speed=0.01, fade_duration_ms=2000):
        ...
        try:
            ...
            pygame.draw.circle(self.original_image, (100, 100, 100, 150), (fallback_diameter // 2, fallback_diameter // 2), fallback_diameter // 2)

        # Масштабирование изображения
        target_ratio = random.uniform(target_size_ratio_range[0], target_size_ratio_range[1])
        ...
        self.scaled_image = pygame.transform.scale(self.original_image, (self.base_width, self.base_height))
        ...
        self.rect.centerx = random.randint(self.rect.width // 2, self.screen_width - self.rect.width // 2)
    ...
    def _update_fade_out(self): # Вызывается из update, если state == 'fade_out'
        ...
        time_elapsed_fade_out = current_time - self.creation_time # creation_time сбрасывается при входе в fade_out

        if time_elapsed_fade_out < self.fade_duration_ms:
            self.alpha = int(255 * (1 - (time_elapsed_fade_out / self.fade_duration_ms)))
        else:
            self.alpha = 0
            self.kill() # Удаляем спрайт из всех групп pygame

        self.alpha = min(max(0, self.alpha), 255) # Убедимся, что альфа в пределах 0-255
        self.image.set_alpha(self.alpha)
    ...
    def update(self): # Полный метод update
        ...
        if self.state == 'fade_in':
            ...
            if time_elapsed_fade_in < self.fade_duration_ms:
                self.alpha = int(255 * (time_elapsed_fade_in / self.fade_duration_ms))
            else:
                self.alpha = 255
                self.state = 'visible'
