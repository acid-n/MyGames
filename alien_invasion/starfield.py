...
class Starfield:
    ...
    def __init__(self, screen, screen_width, screen_height, num_stars_per_layer=150):
        ...
        self.layers_config = [
            {'speed': 0.05, 'color': (100, 100, 100), 'size_range': (1, 1), 'num_stars': self.num_stars_per_layer // 2}, # Дальний слой (медленный, тусклый, мелкий)
            {'speed': 0.10, 'color': (180, 180, 180), 'size_range': (1, 2), 'num_stars': self.num_stars_per_layer}   # Средний слой (быстрее, ярче, чуть крупнее)
        ]
        ...
        for config in self.layers_config:
            ...
            for _ in range(config['num_stars']):
                ...
                stars.append({'x': x, 'y': y, 'initial_y': y, 'size': size, 'speed': config['speed'], 'color': config['color']})
    ...
    def update(self):
        ...
        for layer_idx, stars_in_layer in enumerate(self.layers):
            layer_speed = self.layers_config[layer_idx]['speed'] # Correctly get speed from config
