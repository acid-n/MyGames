...
class PowerUp(Sprite):
    ...
    def __init__(self, ai_game, powerup_type, center_pos):
        ...
        image_path = None
        fallback_color = (128, 128, 128) # Серый цвет по умолчанию для неизвестных типов
        ...
        #     fallback_color = (255, 255, 0) # Желтый для звезды
            ...
            logger.warning(f"Неизвестный тип бонуса '{self.powerup_type}'. Будет использован fallback.")
        ...
        target_size = (self.settings.powerup_display_width, self.settings.powerup_display_height)
        ...
        if image_path and os.path.exists(image_path):
            try:
                ...
                logger.warning(f"Не удалось загрузить изображение бонуса '{image_path}': {e}. Используется fallback.")
                self.image = pygame.Surface(target_size)
                self.image.fill(fallback_color)
        else:
            if image_path: # Путь был указан, но файл не найден
                 logger.warning(f"Файл изображения бонуса не найден: '{image_path}'. Используется fallback.")
