...
def check_assets():
    ...
    try:
        ...
        print(
            f"WARNING: pygame.mixer.init() failed: {e}. Sound checks will be skipped. Attempting to proceed with image checks."
        )
    ...
    project_root = os.path.dirname(
        script_dir
    )  # Assumes tools/ is one level down from root

    sample_image_path = os.path.join(
        project_root, "assets", "gfx", "ui", "icons", "heart.png"
    )
    sample_sound_path = os.path.join(
        project_root, "assets", "audio", "music", "outer_space_loop.ogg"
    )
    placeholder_wav_path = os.path.join(
        project_root, "assets", "audio", "sfx", "laser", "laser01.wav"
    )
    ...
    if not os.path.exists(sample_image_path):
        ...
        if not (50 < size < 200):
            print(
                f"ERROR: Sample image '{sample_image_path}' (1x1 placeholder) has unexpected size: {size} bytes. Expected 50-200 bytes."
            )
        else:
            try:
                ...
                if image.get_width() == 1 and image.get_height() == 1:
                    print(
                        f"Sample image (1x1 placeholder) '{sample_image_path}' loaded successfully and has correct dimensions."
                    )
                    image_placeholder_ok = True
                else:
                    print(
                        f"ERROR: Sample image (placeholder) '{sample_image_path}' loaded but has unexpected dimensions: {image.get_width()}x{image.get_height()}. Expected 1x1."
                    )
            except pygame.error as e:
                print(
                    f"ERROR: Failed to load sample image (placeholder) '{sample_image_path}': {e}"
                )
    ...
    if mixer_initialized:
        ...
        if not os.path.exists(sample_sound_path):
            ...
            if not (30000 < size < 40000):
                print(
                    f"ERROR: Sample sound '{sample_sound_path}' has unexpected size: {size} bytes. Expected 30KB-40KB."
                )
            else:
                try:
                    ...
                    print(
                        f"ERROR: Failed to load sample sound '{sample_sound_path}': {e}"
                    )
        ...
        if not os.path.exists(placeholder_wav_path):
            ...
            if not (30000 < size < 40000):
                print(
                    f"ERROR: Placeholder WAV '{placeholder_wav_path}' has unexpected size: {size} bytes. Expected 30KB-40KB."
                )
            else:
                try:
                    sound = pygame.mixer.Sound(placeholder_wav_path)
                    print(
                        f"Placeholder WAV '{placeholder_wav_path}' loaded successfully."
                    )
                    sound_placeholder_ok = True
                except pygame.error as e:
                    print(
                        f"ERROR: Failed to load placeholder WAV '{placeholder_wav_path}': {e}"
                    )
    else:
    ...
    if not mixer_initialized:
        if image_placeholder_ok:
            ...
            print(
                "Asset check failed (images failed, audio system failed to initialize)."
            )
            sys.exit(1)
