...
# Dependencies:
# For --generate-fallbacks:
# - Pillow: pip install Pillow
# - FFmpeg: must be installed in the system and available in PATH
...
def generate_png_fallback(filepath):
    if not PIL_AVAILABLE:
        print(f"Ошибка: Pillow не установлен. Невозможно создать PNG-заглушку для {filepath}")
        return False
    try:
...
def generate_wav_fallback(filepath):
    try:
        ensure_dir_exists(filepath)
        cmd = ['ffmpeg', '-y', '-f', 'lavfi', '-i', 'anullsrc=channel_layout=mono:sample_rate=44100', '-t', '0.1', filepath]
        ...
        if result.returncode == 0:
            ...
            if result.returncode == 127 or "No such file or directory" in result.stderr or "not found" in result.stderr:
                 print(f"Ошибка: Команда FFmpeg не найдена. Убедитесь, что FFmpeg установлен и в PATH.")
            else:
                print(f"Ошибка: Не удалось создать WAV-заглушку для {filepath} с помощью FFmpeg. Код: {result.returncode}, Ошибка: {result.stderr}")
            return False
    except FileNotFoundError: # Catches if 'ffmpeg' command itself is not found
        print(f"Ошибка: Команда FFmpeg не найдена. Убедитесь, что FFmpeg установлен и в PATH.")
        return False
    except Exception as e:
        print(f"Ошибка: Исключение при попытке создать WAV-заглушку для {filepath}. {e}")
        return False
...
def validate_png(filepath, generate_fallbacks_flag):
    try:
        ...
        if signature == PNG_SIGNATURE:
            ...
            print(f"Ошибка: Неверная PNG сигнатура для {filepath}. Найдено: {signature!r}")
...
def validate_ogg(filepath, generate_fallbacks_flag): # generate_fallbacks_flag not used for OGG yet
    try:
        with open(filepath, 'rb') as f:
            signature = f.read(4)
        if signature == OGG_SIGNATURE:
            ...
            print(f"Ошибка: Неверная OGG сигнатура для {filepath}. Найдено: {signature!r}")
...
def main():
    parser = argparse.ArgumentParser(description="Скрипт валидации ассетов с возможностью генерации заглушек.")
    parser.add_argument("-g", "--generate-fallbacks", action="store_true",
    ...
    if args.generate_fallbacks:
        ...
        if not PIL_AVAILABLE and any(EXPECTED_ASSETS["png"]): # Check if Pillow is needed
            print("Предупреждение: Pillow не найден, генерация PNG-заглушек будет невозможна.")
    ...
    test_png_to_remove_rel = EXPECTED_ASSETS["png"][0] # e.g., assets/gfx/planets/planet01.png
    test_png_to_remove_abs = os.path.join(base_dir, test_png_to_remove_rel)

    test_wav_to_remove_rel = EXPECTED_ASSETS["wav"][0] # e.g., assets/audio/sfx/testing/placeholder.wav
    test_wav_to_remove_abs = os.path.join(base_dir, test_wav_to_remove_rel)

    png_removed_for_test = False
    wav_removed_for_test = False

    if args.generate_fallbacks: # Only do test removals if we are in fallback generation mode
        if os.path.exists(test_png_to_remove_abs):
            os.remove(test_png_to_remove_abs)
            png_removed_for_test = True
            print(f"Тест: Временно удален PNG {test_png_to_remove_abs} для проверки генерации заглушки.")
        else:
            print(f"Тест: PNG {test_png_to_remove_abs} уже отсутствует, подходит для теста генерации.")
        ...
        if os.path.exists(test_wav_to_remove_abs):
             ...
             print(f"Тест: Временно удален WAV {test_wav_to_remove_abs} для проверки генерации заглушки.")
        else:
            print(f"Тест: WAV {test_wav_to_remove_abs} уже отсутствует, подходит для теста генерации.")
    ...
    for asset_type, file_list in EXPECTED_ASSETS.items():
        for relative_path in file_list:
            ...
            if not os.path.exists(filepath):
                ...
                if args.generate_fallbacks:
                    if asset_type == "png":
                        ...
                        else: errors_found_initial +=1 # Fallback failed
                    elif asset_type == "wav":
            ...
            if not asset_ok: # if asset_ok is True, it means fallback was generated and we can skip validation of original
                ...
                if asset_type == "png":
                    ...
                    print(f"Предупреждение: Неизвестный тип ассета '{asset_type}' для файла {filepath}")
                ...
                if not validation_passed:
                    ...
                    if args.generate_fallbacks and (asset_type == "png" or asset_type == "wav"):
                         ...
                         if os.path.exists(filepath): # Check if file exists (could be a fresh fallback)
                            temp_valid = False
                            if asset_type == "png" and validate_png(filepath, False): temp_valid = True # Validate fallback without re-generating
                            elif asset_type == "wav" and validate_wav(filepath, False): temp_valid = True # Validate fallback

                            if temp_valid and errors_found_initial > 0: # if it became valid due to fallback
    ...
    if png_removed_for_test and not os.path.exists(test_png_to_remove_abs):
        print(f"Тест: PNG-файл {test_png_to_remove_abs} не был восстановлен (возможно, ошибка генерации заглушки).")
        ...
        print(f"Тест: PNG-файл {test_png_to_remove_abs} существует (вероятно, заглушка).")

    if wav_removed_for_test and not os.path.exists(test_wav_to_remove_abs):
        print(f"Тест: WAV-файл {test_wav_to_remove_abs} не был восстановлен (возможно, ошибка генерации заглушки).")
    elif wav_removed_for_test and os.path.exists(test_wav_to_remove_abs):
        print(f"Тест: WAV-файл {test_wav_to_remove_abs} существует (вероятно, заглушка).")
    ...
    for asset_type, file_list in EXPECTED_ASSETS.items():
        for relative_path in file_list:
            ...
            if not os.path.exists(filepath):
                ...
                print(f"Критическая ошибка на финальной проверке: Файл ассета НЕ НАЙДЕН - {filepath}")
                final_errors += 1
                continue
            ...
            if not valid_after_fallback:
                print(f"Ошибка на финальной проверке: Невалидный или отсутствующий ассет - {filepath}")
                final_errors +=1
