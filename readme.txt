...
```
**Примечание:** Замените `YOUR_REPOSITORY_URL` на актуальный URL-адрес репозитория. **Git LFS больше не требуется для этого репозитория.** Все необходимые ассеты будут загружены напрямую с помощью команды `git clone`.
...
В случае, если реальные графические (.png) или звуковые (.wav) ассеты отсутствуют в соответствующих директориях (`assets/gfx/`, `assets/audio/`), игра попытается использовать минимально валидные файлы-заглушки. Это сделано для того, чтобы игра могла запуститься и функционировать без критических ошибок загрузки ресурсов, даже если полный набор ассетов не был загружен или предоставлен.

*   **Для PNG изображений:** Используются автоматически сгенерированные прозрачные изображения размером 1x1 пиксель. Это позволяет избежать ошибок "Unsupported image format", которые могли бы возникнуть с невалидными или нулевыми файлами.
*   **Для WAV аудиофайлов:** В качестве заглушек используются копии существующего валидного OGG аудиофайла (`assets/audio/music/outer_space_loop.ogg`), переименованные в `.wav`. Это предпринято для предотвращения ошибок "Couldn't read audio file", которые могут возникнуть при попытке загрузить некорректные или пустые WAV файлы. Звучание таких заглушек не будет соответствовать ожидаемому игровому событию, но позволит аудиосистеме инициализироваться и игре работать.

Рекомендуется заменять эти заглушки реальными игровыми ассетами для полноценного визуального и звукового опыта. Скрипт `download_assets.sh` содержит инструкции и ссылки для скачивания большинства необходимых ассетов.
